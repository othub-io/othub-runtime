...
router.post("/", web3passport.authenticate('jwt', { session: false }), async function (req, res, next) {
    ...
    if (data.app_description && data.app_description !== '') {
        ...
        await queryDB.getData(query, [data.app_description, app_name], "", "othub_db")
        .then((results) => {
    ...
    if (data.built_by && data.built_by !== '') {
        ...
        await queryDB.getData(query, [data.built_by, app_name], "", "othub_db")
        .then((results) => {
            //console.log('Query results:', results);
            return results;
            // Use the results in your variable or perform further operations
    ...
    if (data.website && data.website !== '') {
        ...
        await queryDB.getData(query, [data.website, app_name], "", "othub_db")
        .then((results) => {
            //console.log('Query results:', results);
            return results;
            // Use the results in your variable or perform further operations
    ...
    if (data.github && data.github !== '') {
        ...
        await queryDB.getData(query, [data.github, app_name], "", "othub_db")
        .then((results) => {
            //console.log('Query results:', results);
            return results;
            // Use the results in your variable or perform further operations
